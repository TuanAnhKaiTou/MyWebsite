...
{
global $upload_dir,$max_file_size_upload,
$thumb_dir,$thumbnail_height,$thumbnail_width,
$small_dir,$small_height,$small_width;
...
else
{
    ...
    if (move_uploaded_file($_FILES[$filename]["tmp_name"], $target_dir.$newfilename))
	{
		...
		return $newfilename.=$newfliename."|";
    }
	else
	{
        $newfilename="";
...
if($stt==$i)
$rs="<img src='".$small_dir.$each_img_name[$i]."' alt='' />";
else if($stt<0)
$rs.="<img src='".$small_dir.$each_img_name[$i]."' alt='' />";
return $rs;
}
...
{
global $upload_dir,$large,$small,$thumb,$stt;
$each_img_name=explode("|",$name);$rs="";
for($i=0;$i<count($each_img_name);$i++)
{
if($i==0)
{
$rs="<img id='zoom' src='".$upload_dir.$small.$each_img_name[$i]."' 
data-zoom-image='".$upload_dir.$large.$each_img_name[$i]."'/>
<div class='owl-carousel' id='gallery'>";
}
$rs.="<a href='#' data-image='".$upload_dir.$small.$each_img_name[$i]."' 
data-zoom-image='".$upload_dir.$large.$each_img_name[$i]."'>
<img id='zoom' src='".$upload_dir.$thumb.$each_img_name[$i]."' />
</a>";
}
$rs.="</div>";
return $rs;
